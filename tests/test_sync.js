...
test('logout restaura área anônima limpa, nunca snapshot remoto por acidente',()=>{assert.match(engine,/ensureAnonymousBaseline/);assert.match(engine,/saveAnonymousSnapshot\(JSON\.stringify\(AppCore\.createInitialState\(\)\)\)/);assert.doesNotMatch(engine,/saveAnonymousSnapshot\(Store\.raw\(\)\)/);assert.match(engine,/readAnonymousSnapshot\(\)/);});
