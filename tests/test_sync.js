...
test('estado significativo',()=>{assert.equal(Core.isMeaningfulState(sample()),true);assert.equal(Core.isMeaningfulState({vehicles:[{plate:'',odometer:0}],transactions:[],reminders:[],odometerLogs:[],preferences:{}}),false);assert.equal(Core.isMeaningfulState({vehicles:[{id:'1'},{id:'2'}],transactions:[],reminders:[],odometerLogs:[],preferences:{}}),true);assert.equal(Core.isMeaningfulState({vehicles:[{id:'1'}],transactions:[],reminders:[{byKm:true}],odometerLogs:[],preferences:{}}),true);});
...
test('backup local antes de substituição',()=>{assert.match(storage,/motofinance_sync_last_local_backup/);assert.match(storage,/replaceState/);assert.match(storage,/if\s*\(!set\(BACKUP,\s*backup\)\)\s*return false/);});
